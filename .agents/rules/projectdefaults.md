...
- Coil Compose: `2.6.0`
- Rich Text Editor: `com.mohamedrejeb.richeditor:richeditor-compose:1.0.0-rc05-k2`
...
- **Stateless/Stateful Separation**: Every screen must decouple the stateful container (`Screen(viewModel: ViewModel)`) from the stateless content (`ScreenContent(uiState: UiState, onAction: (Action) -> Unit)`).
- **Mandatory Previews for New Screens**: Every newly added screen MUST have `@Preview` composables defined for visual verification in Android Studio.
- **Multiple State Previews**: Provide previews for essential UI states:
...
### Unit Tests
- Cover 80%+ of domain and data layers
